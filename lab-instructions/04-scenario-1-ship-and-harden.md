...
If you’re not already in the **devops-dashboard** directory, cd into it, then prompt `copilot` to start a Copilot session. Then, say to copilot: 
...
If the generated Bicep already includes system-assigned managed identity with AcrPull, skip the above prompt and instead ask Copilot how-to questions about a gap that remains:
...
```
How do I add an HTTP health probe to my Container App that uses the /health endpoint."
```
